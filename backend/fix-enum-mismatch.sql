-- Fix for enum mismatch between PostgreSQL seed data and Java enums.
-- Run in inteko_db: psql -U nelson -d inteko_db -f backend/fix-enum-mismatch.sql
--
-- IMPORTANT: Must drop check constraints BEFORE updating values.
-- Updating to 'SCHEDULED' while constraint only allows 'Scheduled' will fail.
...
-- Step 2: Update values to match Java enums
...
-- Verify
SELECT 'user_accounts.role' AS field, role AS value, COUNT(*) FROM user_accounts GROUP BY role
UNION ALL
SELECT 'meetings.status', status, COUNT(*) FROM meetings GROUP BY status;
